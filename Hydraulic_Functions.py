...
def pressure_loss_shell_finder(reynolds_shell, N, v_shell, a, baffles):
    rho = 990.1
    fs = 0.2 * reynolds_shell ** (-0.15)
    pressure_loss_shell = rho * (v_shell)**2 * (baffles + 1) *fs #+ 4*a*(reynolds_shell**(-0.15))*N*rho*v_shell**2
...
def pressure_intersection_1(N, Nb, Y, L, mdot_upper=0.8):
    ...
    for mdot in mdot1:
    #Cold stream
        A_sh = shell_area_finder(Y, Nb, L) * 2 #changed to see
...
def H_finder(reynolds_tube, reynolds_shell, L, N_b, Y, mdot1, passes, N):
    L = L*passes*N
    Pr = 4.31
...
def temperature_solvernew2(mdot1, mdot2, H, A, F, N_i, passes):
    shells = passes/2
    T1in = 20
    T2in = 60
    cp = 4179
    ...
    best_delta_q23 = 1e6
    fudge = 10
    step = (T2out-T1out)/N_i
    T_List = np.arange(T1out, T2out, step)
    for T2out in T_List:
        T1out = T1in + (mdot2/mdot1)*(T2in - T2out)
